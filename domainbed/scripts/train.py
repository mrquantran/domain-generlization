...
if __name__ == "__main__":
    ...
    for step in tqdm(range(start_step, n_steps), desc="Training Progress"):
        ...
        if (step % checkpoint_freq == 0) or (step == n_steps - 1):
            ...
            loss = results["loss"]
            ssl_loss = results["ssl_loss"]
            class_loss = results["class_loss"]
            contrastive_loss = results["contrastive_loss"]
            glo_loss = results["glo_loss"]
            misc.print_row(["loss", loss, "ssl_loss", ssl_loss, "class_loss", class_loss, "contrastive_loss", contrastive_loss, "glo_loss", glo_loss], colwidth=12)
